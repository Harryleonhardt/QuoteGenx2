...
# --- FIRESTORE SETUP ---
# This function initializes the connection to the Firestore database.
# It uses credentials stored securely in Streamlit's secrets management.
def init_firestore_client():
    """Initializes and returns a Firestore client if credentials are valid."""
    try:
        # The service account key is stored as a JSON string in Streamlit secrets.
        key_dict = json.loads(st.secrets["textkey"])
        creds = service_account.Credentials.from_service_account_info(key_dict)
        db = firestore.Client(credentials=creds)
        return db
    except (KeyError, json.JSONDecodeError):
        # This error shows if the secrets are not set up correctly.
        st.error("🚨 Firestore credentials not found or invalid. Please follow the setup instructions.", icon="🚨")
        return None
