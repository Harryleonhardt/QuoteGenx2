...
if not st.session_state.quote_items.empty:
    ...
    with st.container(border=False):
        st.markdown('<div class="step-container">', unsafe_allow_html=True)
        # We use a form here to batch the final inputs for PDF generation
        with st.form("quote_details_form"):
            st.header("Step 3: Project & Customer Details")
            
            st.subheader("Load Customer Profile (Optional)")
            # This widget is outside the form, so it works immediately
            customer_profile_zip = st.file_uploader(
                "Upload Customer Profile (.zip)", 
                type="zip", 
                help="Upload a .zip file containing details.json and a logo."
            )
            # This processing logic is also outside the form
            ...
            # We don't need a separate file uploader for the logo inside the form
            # It's handled by the zip upload now. We can show a preview if it exists.
        ...
        if submitted:
            ...
            quote_to_html = f"""
                ...
                <p class="text-gray-700 mt-2"><strong class="font-bold text-gray-800">Attn:</strong> {q_details["attention"] or "N/A"}</p>
            """
            ...
            st.download_button(
                label="✅ Download Final Quote as PDF", data=pdf_bytes, file_name=f"Quote_{q_details['quoteNumber']}_{q_details['customerName']}.pdf",
